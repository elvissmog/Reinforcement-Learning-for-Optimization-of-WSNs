...
class Yamada(object):
    ...
    Attributes:
        graph (nx.Graph): graph to find all minimum spanning trees for.        trees (list, nx.Graph): list of discovered minimum spanning trees.
    ...
    def replace_edge(self, tree, old_edge, new_edge):
        """
        Replace an edge in a tree with a substitute edge. Running of subsitution algorithm
...
class Substitute(object):
    ...
    def postorder_tree(self):
        ...
        mappings for all nodes within a tree. Source node is defined by        self.source_node and defaults to the lexigraphically last node.
    ...
    def postordered_edges(self):
        ...
        for u, v in self.tree.edges():
            # ensure edges are orders (u, v) such that u has the lowest            # postorder

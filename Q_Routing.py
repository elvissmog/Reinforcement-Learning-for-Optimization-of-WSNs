...
for elem in node_neigh:
    ...
    for index in node_neigh[elem]:
        dis = math.sqrt(math.pow((position_array[elem][0] - position_array[index][0]), 2) + math.pow((position_array[elem][1] - position_array[index][1]), 2))
...
def path_qval(node):
    if not node in G.nodes:
        ...
        new_path_qval = {}
        for elem in neighs:
            dis = math.sqrt(math.pow((position_array[node][0] - position_array[elem][0]), 2) + math.pow((position_array[node][1] - position_array[elem][1]), 2))
...
for i in range(len(G)):
	for j in range(len(G)):
		if i !=j:
			R[i][j] = math.sqrt(math.pow((position_array[i][0] - position_array[j][0]), 2) + math.pow((position_array[i][1] - position_array[j][1]), 2))

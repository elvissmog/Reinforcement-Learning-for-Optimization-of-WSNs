        ...
        if node != sink_node:
            ...
            while True:
                ...
                for neigh in node_neighbors[start]:
                    Eval_neigh.append(e_values[neigh])

                for neigh in node_neighbors[start]:

                    if max(Eval_neigh) == min(Eval_neigh):
                        all_path_rwds[node][start][neigh] = 1 / h_counts[neigh]

                    if max(Eval_neigh) != min(Eval_neigh):
                        all_path_rwds[node][start][neigh] = prob*((e_values[node])-min(Eval_neigh)/(max(Eval_neigh)-min(Eval_neigh)))+((1-prob)/h_counts[neigh])

                    if e_values[node]<=node_energy_limit:
                        all_path_rwds[node][start][neigh] = -100

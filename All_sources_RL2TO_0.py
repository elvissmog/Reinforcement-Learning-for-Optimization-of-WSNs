    ...
    for node in range(len(G.nodes)-1):
        ...
        while True:
            ...
            for n in node_neigh[s]:
                dss = math.ceil(
                    math.sqrt(math.pow((p[s][0] - p[sink_node][0]), 2) + math.pow((p[s][1] - p[sink_node][1]), 2)))
                dsn = math.ceil(math.sqrt(math.pow((p[s][0] - p[n][0]), 2) + math.pow((p[s][1] - p[n][1]), 2)))
                if dsn <= d_o:
                    etx = electronic_energy * data_packet_size + e_fs * data_packet_size * math.pow(dsn, 2)
                else:
                    etx = electronic_energy * data_packet_size + e_fs * data_packet_size * math.pow(dsn, 4)
                tp.append(etx)

                er = etx/max(tp)
                rwd = E_vals[n] / (er * hop_counts[n])
                #rwd = E_vals[n] /hop_counts[n]

                temp_qval[n] = (1 - learning_rate) * path_Q_values[node][s][n] + learning_rate * (rwd + Q_vals[node][n])
            ...
            path = path + "->" + str(nh)  # update the path after each visit
...
with open('nrlttovals.txt', 'w') as f:
    f.write(json.dumps(NQ))

# Now read the file back into a Python list object
with open('nrlttovals.txt', 'r') as f:

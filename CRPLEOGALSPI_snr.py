...
def build_graph(positions, links):
    ...
    phi_matrix = np.zeros((len(all_msts), len(all_msts)))
    Q_matrix = np.zeros((len(all_msts), len(all_msts)))

    for s in range(len(all_msts)):
        for a in range(len(all_msts)):
            EC = []
            for node in MST_paths[a]:
                count = 0
                while count < len(MST_paths[a][node]) - 1:
                    ini = MST_paths[a][node][count]
                    nex = MST_paths[a][node][count + 1]
                    dis = math.ceil(math.sqrt(math.pow((position_array[ini][0] - position_array[nex][0]), 2) + math.pow((position_array[ini][1] - position_array[nex][1]), 2)))
                    # print('dis:', dis)
                    if dis <= d_o:
...
for rdn in range(episodes):
    ...
    for samp in range(len(Samples)):
        phi = PHI_matrix[Samples[samp][0], Samples[samp][1]]
        act = np.argmin(PHI_matrix[Samples[samp][1], :])
        phi_next = PHI_matrix[Samples[samp][1], act]
        r = Samples[samp][3]
